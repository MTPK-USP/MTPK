...
if sel_fun_data:
    ...
    if do_galaxies:
        ...
        gridx , gridy, gridz = grid.RX , grid.RY , grid.RZ
        ...
        for ng in range(ngals):
            ...
            # Exponential form:
            nbar_sel_fun = selection_func(grid.grid_r, nbar[ng],ncentral[ng],nsigma[ng])
            # Linear form:
            #nbar_sel_fun = selection_func(gridx, gridy, gridz, nbar[ng],ax[ng],ay[ng],az[ng])
            # Use this to build galaxy hod, cell by cell, given the halo mass function, for each halo:
            for nh in range(nhalos):
                hod_3D[ng,nh] = mult_sel_fun*np.asarray(hod[ng,nh]/nbar[ng] * nbar_sel_fun , dtype='float32')
...
#pk_mg = pkmg.pkmg(halo_bias,dip,matgrowcentral,k_camb,len(halo_bias)*[0],a_sig_tot,cH,zcentral)
...
try:
    #pk_mg_cross = pkmg_cross.pkmg_cross(halo_bias,dip,matgrowcentral,k_camb,len(halo_bias)*[0],a_sig_tot,cH,zcentral)
...
try:
    ...
    pk_mg_data = pkmg.pkmg(data_bias,dip,matgrowcentral,k_camb,a_sig_tot,cH,zcentral)
...
#pk_mg = pkmg.pkmg(gal_bias,gal_adip,matgrowcentral,k_camb,len(gal_bias)*[0],a_gal_sig_tot,cH,zcentral)
